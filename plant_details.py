...
class PlantDetails(ctk.CTkFrame):
    ...
    def plant_details_screen(self) -> None:
        ...
        self.img_label.grid(row=1, column=0, padx=20, pady=10, rowspan=4)
        ...
        self.supstrate = ctk.CTkLabel(self, text=f"Dodavanje supstrata: {'da' if self.plant[6] == 1 else 'ne'}", font=ctk.CTkFont(size=14))
        self.supstrate.grid(row=4, column=1, padx=10, sticky="we", columnspan=2)

        self.editbtn = ctk.CTkButton(self, text="Ažuriraj podatke o biljci", command=self.toggle_edit_mode)
        self.editbtn.grid(row=5, column=0, pady=20, padx=10, sticky="we", columnspan=3)
    ...
    def details_editing(self) -> None:
        ...
        self.supstrate.grid(row=4, column=1, padx=10, sticky="w")
        self.entry_substrate = ctk.CTkComboBox(self, values=["da", "ne"])
        self.entry_substrate.set(f"{'da' if self.plant[6] == 1 else 'ne'}")
        self.entry_substrate.grid(row=4, column=2, padx=10, sticky="we")
